...
class TSPSolver(object):
    ...
    def __init__(self, nodes, graph, shifts_amount, delete_ok):
        """
    ...
    def get_obj(self):
        ...
        if self.delete_ok:
            for i in range(self.graph_size):
                for j in range(self.graph_size):
                    obj[self.get_xij_index(i, j)] -= 200000
    ...
    def add_outgoing_constraints(self):
        ...
        self.solver.linear_constraints.add(lin_expr=rows, senses=['L' if self.delete_ok else 'E'] * len(rows), rhs=[1] * len(rows))
    ...
    def reconstruct_path(self, edges):
        ...
        while len(edges_path) < len(edges):
            for edge in edges:
